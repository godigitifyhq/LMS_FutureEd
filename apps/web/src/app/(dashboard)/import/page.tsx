    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < raw.length; i++) {
          ...
          if (!parsed['studentName'] && !parsed['phone']) continue
...
export default function ImportPage() {
  ...
  const [duplicateActions, setDuplicateActions] = useState<Record<number, 'skip' | 'merge'>>({})
  ...
  function downloadTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([
      ['studentName', 'phone', 'email', 'fatherName', 'city', 'state', 'qualification'],
      ['John Doe', '9876543210', 'john@email.com', 'Robert Doe', 'Bokaro', 'Jharkhand', 'TWELFTH'],
    ])
      ...
      {file && !result && (
        <div className="space-y-4">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-surface-100 bg-surface-50">
                      {['#', 'Student Name', 'Phone', 'Email', 'Father Name', 'City', 'Status'].map((h) => (
                  ...
                  <tbody className="divide-y divide-surface-50">
                    {validRows.slice(0, 10).map((row) => (
                      <tr key={row.rowIndex} className="hover:bg-surface-50">
                        ...
                        <td className="px-3 py-2 text-sm font-medium text-gray-800">{row.studentName}</td>
                        <td className="px-3 py-2 text-sm text-gray-600">{row.phone}</td>
                        <td className="px-3 py-2 text-xs text-gray-500">{row.email ?? '—'}</td>
                        <td className="px-3 py-2 text-xs text-gray-500">{row.fatherName ?? '—'}</td>
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-sm font-semibold text-red-700">
                  {invalidRows.length} rows missing required fields (Name + Phone) — will be skipped
                </p>
              </div>
              <div className="text-xs text-red-600 space-y-1">
                {invalidRows.slice(0, 5).map((r) => (
                  <p key={r.rowIndex}>Row {r.rowIndex}: Missing {!r.studentName ? 'Student Name' : 'Phone'}</p>
                ))}
          ...
          <div className="flex gap-3 justify-end">
            <Button variant="secondary" onClick={() => { setFile(null); setParsedRows([]) }}>
              Cancel
            </Button>
            <Button
              ...
              <div className="divide-y divide-surface-100">
                {result.duplicateQueue.map((item) => (
                  <div key={item.rowIndex} className="px-4 py-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-gray-800">{item.rowData.studentName}</p>
                      <p className="text-xs text-gray-500">{item.rowData.phone} · Matched by {item.matchType}</p>
                    </div>
                    <a href={`/leads/${item.existingLeadId}`}
